...
// Main methods
RedisModel.prototype.uniqId = function (hash_, callback) {
  this.db.incr(this.kCounter(), function (err, reply) {
    console.log('model:uniqId:hash_: ' + hash_);
    console.log('model:uniqId:reply: ' + reply);
    if (hash_ == "" || hash_ == null || hash_ == undefined) {
      hash_ = base58.encode(getRandomInt(9999, 999999) + reply.toString());
    }
...
// Set record
RedisModel.prototype.set = function (hash_, long_url, callback) {
  var self = this;
  this.findUrl(long_url, function (err, reply) {
    if (err) {
      ...
      console.log('model:set:hash_: ' + hash_);
      self.uniqId(hash_, function (err, hash_) {
        if (err) {
          conosle.log(err);
          callback(500);
          self.db.quit();
